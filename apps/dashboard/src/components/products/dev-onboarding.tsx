...
export type DevOnboardingProps = {
  ...
  /**
   * When true, render without the component's own card chrome (border,
   * gradient background, shadow, padding) and without the duplicated
   * "Get started" heading + intro paragraph. Use this when the consumer
   * already provides a surrounding card and section heading (e.g. the
   * product dashboard's collapsible install panel).
   */
  bare?: boolean;
};
  ...
  const resourceBlock = (
    <div className="mt-5 grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <ResourceLink
        icon={ScrollText}
        label="Changelog"
        description="Per-source log in the dashboard"
