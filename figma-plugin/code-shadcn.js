// ─── COLOR PRIMITIVE ───────────────────────────────────────────────────────

const colors = {
  primary: { 50:'#FDF2F6',100:'#F7CCDF',200:'#E9669F',300:'#FF7AB4',400:'#E2337E',500:'#DB005F',600:'#B5004F',700:'#7B0036',800:'#7A0035',900:'#3F0D23',950:'#260815' },
  accentPrimary: { 50:'#F4F5FA',100:'#EEEFF3',200:'#DDE0F3',300:'#C8DFE7',500:'#8B95EE',600:'#3F4FED',700:'#181E5A',800:'#131746',900:'#0D0E2C' },
  accentSecondary: { 100:'#F9DAD8',200:'#F9C1C0',300:'#FA716B',500:'#FA433B',600:'#DE271F',700:'#BD2C27',800:'#801914' },
  slate:   { 50:'#f8fafc',100:'#f1f5f9',200:'#e2e8f0',300:'#cbd5e1',400:'#94a3b8',500:'#64748b',600:'#475569',700:'#334155',800:'#1e293b',900:'#0f172a',950:'#020617' },
  gray:    { 50:'#f9fafb',100:'#f3f4f6',200:'#e5e7eb',300:'#d1d5db',400:'#9ca3af',500:'#6b7280',600:'#4b5563',700:'#374151',800:'#1f2937',900:'#111827',950:'#030712' },
  zinc:    { 50:'#fafafa',100:'#f4f4f5',200:'#e4e4e7',300:'#d4d4d8',400:'#a1a1aa',500:'#71717a',600:'#52525b',700:'#3f3f46',800:'#27272a',900:'#18181b',950:'#09090b' },
  neutral: { 50:'#fafafa',100:'#f5f5f5',200:'#e5e5e5',300:'#d4d4d4',400:'#a3a3a3',500:'#737373',600:'#525252',700:'#404040',800:'#262626',900:'#171717',950:'#0a0a0a' },
  stone:   { 50:'#fafaf9',100:'#f5f5f4',200:'#e7e5e4',300:'#d6d3d1',400:'#a8a29e',500:'#78716c',600:'#57534e',700:'#44403c',800:'#292524',900:'#1c1917',950:'#0c0a09' },
  red:     { 50:'#fef2f2',100:'#fee2e2',200:'#fecaca',300:'#fca5a5',400:'#f87171',500:'#ef4444',600:'#dc2626',700:'#b91c1c',800:'#991b1b',900:'#7f1d1d',950:'#450a0a' },
  orange:  { 50:'#fff7ed',100:'#ffedd5',200:'#fed7aa',300:'#fdba74',400:'#fb923c',500:'#f97316',600:'#ea580c',700:'#c2410c',800:'#9a3412',900:'#7c2d12',950:'#431407' },
  amber:   { 50:'#fffbeb',100:'#fef3c7',200:'#fde68a',300:'#fcd34d',400:'#fbbf24',500:'#f59e0b',600:'#d97706',700:'#b45309',800:'#92400e',900:'#78350f',950:'#451a03' },
  yellow:  { 50:'#fefce8',100:'#fef9c3',200:'#fef08a',300:'#fde047',400:'#facc15',500:'#eab308',600:'#ca8a04',700:'#a16207',800:'#854d0e',900:'#713f12',950:'#422006' },
  lime:    { 50:'#f7fee7',100:'#ecfccb',200:'#d9f99d',300:'#bef264',400:'#a3e635',500:'#84cc16',600:'#65a30d',700:'#4d7c0f',800:'#3f6212',900:'#365314',950:'#1a2e05' },
  green:   { 50:'#f0fdf4',100:'#dcfce7',200:'#bbf7d0',300:'#86efac',400:'#4ade80',500:'#22c55e',600:'#16a34a',700:'#15803d',800:'#166534',900:'#14532d',950:'#052e16' },
  emerald: { 50:'#ecfdf5',100:'#d1fae5',200:'#a7f3d0',300:'#6ee7b7',400:'#34d399',500:'#10b981',600:'#059669',700:'#047857',800:'#065f46',900:'#064e3b',950:'#022c22' },
  teal:    { 50:'#f0fdfa',100:'#ccfbf1',200:'#99f6e4',300:'#5eead4',400:'#2dd4bf',500:'#14b8a6',600:'#0d9488',700:'#0f766e',800:'#115e59',900:'#134e4a',950:'#042f2e' },
  cyan:    { 50:'#ecfeff',100:'#cffafe',200:'#a5f3fc',300:'#67e8f9',400:'#22d3ee',500:'#06b6d4',600:'#0891b2',700:'#0e7490',800:'#155e75',900:'#164e63',950:'#083344' },
  sky:     { 50:'#f0f9ff',100:'#e0f2fe',200:'#bae6fd',300:'#7dd3fc',400:'#38bdf8',500:'#0ea5e9',600:'#0284c7',700:'#0369a1',800:'#075985',900:'#0c4a6e',950:'#082f49' },
  blue:    { 50:'#eff6ff',100:'#dbeafe',200:'#bfdbfe',300:'#93c5fd',400:'#60a5fa',500:'#3b82f6',600:'#2563eb',700:'#1d4ed8',800:'#1e40af',900:'#1e3a8a',950:'#172554' },
  indigo:  { 50:'#eef2ff',100:'#e0e7ff',200:'#c7d2fe',300:'#a5b4fc',400:'#818cf8',500:'#6366f1',600:'#4f46e5',700:'#4338ca',800:'#3730a3',900:'#312e81',950:'#1e1b4b' },
  violet:  { 50:'#f5f3ff',100:'#ede9fe',200:'#ddd6fe',300:'#c4b5fd',400:'#a78bfa',500:'#8b5cf6',600:'#7c3aed',700:'#6d28d9',800:'#5b21b6',900:'#4c1d95',950:'#2e1065' },
  purple:  { 50:'#faf5ff',100:'#f3e8ff',200:'#e9d5ff',300:'#d8b4fe',400:'#c084fc',500:'#a855f7',600:'#9333ea',700:'#7e22ce',800:'#6b21a8',900:'#581c87',950:'#3b0764' },
  fuchsia: { 50:'#fdf4ff',100:'#fae8ff',200:'#f5d0fe',300:'#f0abfc',400:'#e879f9',500:'#d946ef',600:'#c026d3',700:'#a21caf',800:'#86198f',900:'#701a75',950:'#4a044e' },
  pink:    { 50:'#fdf2f8',100:'#fce7f3',200:'#fbcfe8',300:'#f9a8d4',400:'#f472b6',500:'#ec4899',600:'#db2777',700:'#be185d',800:'#9d174d',900:'#831843',950:'#500724' },
  rose:    { 50:'#fff1f2',100:'#ffe4e6',200:'#fecdd3',300:'#fda4af',400:'#fb7185',500:'#f43f5e',600:'#e11d48',700:'#be123c',800:'#9f1239',900:'#881337',950:'#4c0519' },
};

function hexToRgb(hex) {
  return {
    r: parseInt(hex.slice(1,3),16)/255,
    g: parseInt(hex.slice(3,5),16)/255,
    b: parseInt(hex.slice(5,7),16)/255,
    a: 1,
  };
}

const colorCollection = figma.variables.createVariableCollection('Color Primitive');
const colorModeId = colorCollection.defaultModeId;

const colorEntries = [
  ['black', '#000000'],
  ['white', '#ffffff'],
  ...Object.entries(colors).flatMap(([name, shades]) =>
    Object.entries(shades).map(([shade, hex]) => [`${name}/${shade}`, hex])
  ),
];

for (const [name, hex] of colorEntries) {
  const v = figma.variables.createVariable(name, colorCollection, 'COLOR');
  v.setValueForMode(colorModeId, hexToRgb(hex));
}

// ─── TYPOGRAPHY PRIMITIVE ──────────────────────────────────────────────────

const fontSizes = {
  xs:    12,
  sm:    14,
  base:  16,
  lg:    18,
  xl:    20,
  '2xl': 24,
  '3xl': 30,
  '4xl': 36,
  '5xl': 48,
  '6xl': 60,
  '7xl': 72,
  '8xl': 96,
  '9xl': 128,
};

const fontWeights = {
  thin:       100,
  extralight: 200,
  light:      300,
  normal:     400,
  medium:     500,
  semibold:   600,
  bold:       700,
  extrabold:  800,
  black:      900,
};

const lineHeights = {
  none:    1,
  tight:   1.25,
  snug:    1.375,
  normal:  1.5,
  relaxed: 1.625,
  loose:   2,
};

// Stored as px values (calculated at 16px base)
const letterSpacing = {
  tighter: -0.8,
  tight:   -0.4,
  normal:   0,
  wide:     0.4,
  wider:    0.8,
  widest:   1.6,
};

const fontFamilies = {
  sans: 'Inter, ui-sans-serif, system-ui, sans-serif',
  mono: 'ui-monospace, SFMono-Regular, Menlo, Monaco, monospace',
};

const typoCollection = figma.variables.createVariableCollection('Typography Primitive');
const typoModeId = typoCollection.defaultModeId;

function float(name, value) {
  const v = figma.variables.createVariable(name, typoCollection, 'FLOAT');
  v.setValueForMode(typoModeId, value);
}

function string(name, value) {
  const v = figma.variables.createVariable(name, typoCollection, 'STRING');
  v.setValueForMode(typoModeId, value);
}

for (const [k, v] of Object.entries(fontSizes))     float(`font-size/${k}`, v);
for (const [k, v] of Object.entries(fontWeights))   float(`font-weight/${k}`, v);
for (const [k, v] of Object.entries(lineHeights))   float(`line-height/${k}`, v);
for (const [k, v] of Object.entries(letterSpacing)) float(`letter-spacing/${k}`, v);
for (const [k, v] of Object.entries(fontFamilies))  string(`font-family/${k}`, v);

// ─── TAILWIND SPEC ──────────────────────────────────────────────────────────

const tailwindSpec = {
  'Flexbox & Grid': [
    'flex-basis', 'flex-direction', 'flex-wrap', 'flex', 'flex-grow', 'flex-shrink', 'order',
    'grid-template-columns', 'grid-column', 'grid-template-rows', 'grid-row', 'grid-auto-flow', 'grid-auto-columns', 'grid-auto-rows',
    'gap',
    'justify-content', 'justify-items', 'justify-self',
    'align-content', 'align-items', 'align-self',
    'place-content', 'place-items', 'place-self',
  ],
  Spacing: ['padding', 'margin'],
  Sizing: [
    'width', 'min-width', 'max-width', 'height', 'min-height', 'max-height',
    'inline-size', 'min-inline-size', 'max-inline-size',
    'block-size', 'min-block-size', 'max-block-size',
  ],
  Typography: [
    'font-family', 'font-size', 'font-smoothing', 'font-style', 'font-weight', 'font-stretch',
    'font-variant-numeric', 'font-feature-settings', 'letter-spacing', 'line-clamp', 'line-height',
    'list-style-image', 'list-style-position', 'list-style-type',
    'text-align', 'color',
    'text-decoration-line', 'text-decoration-color', 'text-decoration-style', 'text-decoration-thickness',
    'text-underline-offset', 'text-transform', 'text-overflow', 'text-wrap', 'text-indent',
    'vertical-align', 'white-space', 'word-break', 'overflow-wrap', 'hyphens', 'content',
  ],
  Backgrounds: [
    'background-attachment', 'background-clip', 'background-color', 'background-image',
    'background-origin', 'background-position', 'background-repeat', 'background-size',
  ],
  Borders: [
    'border-radius', 'border-width', 'border-color', 'border-style',
    'outline-width', 'outline-color', 'outline-style', 'outline-offset',
  ],
};
const tailwindOfficialNumeric = {
  spacing: { '0':0,'0.5':2,'1':4,'1.5':6,'2':8,'2.5':10,'3':12,'3.5':14,'4':16,'5':20,'6':24,'7':28,'8':32,'9':36,'10':40,'11':44,'12':48,'14':56,'16':64,'20':80,'24':96,'28':112,'32':128,'36':144,'40':160,'44':176,'48':192,'52':208,'56':224,'60':240,'64':256,'72':288,'80':320,'96':384, px:1 },
  borderRadius: { none:0, sm:2, DEFAULT:4, md:6, lg:8, xl:12, '2xl':16, '3xl':24, full:9999 },
  borderWidth: { '0':0, DEFAULT:1, '2':2, '4':4, '8':8 },
  fontSize: { xs:12, sm:14, base:16, lg:18, xl:20, '2xl':24, '3xl':30, '4xl':36, '5xl':48, '6xl':60, '7xl':72, '8xl':96, '9xl':128 },
  lineHeight: { '3':12, '4':16, '5':20, '6':24, '7':28, '8':32, '9':36, '10':40 },
  maxWidth: { xs:320, sm:384, md:448, lg:512, xl:576, '2xl':672, '3xl':768, '4xl':896, '5xl':1024, '6xl':1152, '7xl':1280 },
  outlineOffset: { '0':0, '1':1, '2':2, '4':4, '8':8 },
  letterSpacing: { tighter:-0.05, tight:-0.025, normal:0, wide:0.025, wider:0.05, widest:0.1 },
};

const specCollection = figma.variables.createVariableCollection('Tailwind Spec');
const specModeId = specCollection.defaultModeId;
let specCount = 0;
function utilitySlug(name) {
  return String(name).toLowerCase().replace(/[^a-z0-9.-]+/g, '-').replace(/^-+|-+$/g, '');
}

for (const [scale, values] of Object.entries(tailwindOfficialNumeric)) {
  for (const [token, value] of Object.entries(values)) {
    const v = figma.variables.createVariable(`tw/${scale}/${utilitySlug(token)}`, specCollection, 'FLOAT');
    v.setValueForMode(specModeId, value);
    specCount++;
  }
}

const basisTokens = {
  '3xs': 256, '2xs': 288, xs: 320, sm: 384, md: 448, lg: 512, xl: 576,
  '2xl': 672, '3xl': 768, '4xl': 896, '5xl': 1024, '6xl': 1152, '7xl': 1280,
};
for (const [token, px] of Object.entries(basisTokens)) {
  const v = figma.variables.createVariable(`tw/flex-basis/basis-${token}`, specCollection, 'FLOAT');
  v.setValueForMode(specModeId, px);
  specCount++;
}

// ─── DONE ──────────────────────────────────────────────────────────────────

const typoCount = [fontSizes, fontWeights, lineHeights, letterSpacing, fontFamilies]
  .reduce((sum, obj) => sum + Object.keys(obj).length, 0);

figma.notify(`Created ${colorEntries.length} color + ${typoCount} typography + ${specCount} spec variables`);
figma.closePlugin();
