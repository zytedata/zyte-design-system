/**
 * Schema-tolerant codegen for `@zyte/ds-*` packages.
 *
...
import type { ProductBranding, ProductFoundations } from "@zyte/ds-types";

// ---------------------------------------------------------------------------
// Slug + export-name conventions

const SCOPED_NAME_RE = /^@zyte\/ds-([a-z][a-z0-9-]*)$/;

export function slugFromPackageName(name: string): string {
  const match = SCOPED_NAME_RE.exec(name);
  if (!match) {
    throw new Error(
      `tokens-build: package "${name}" must be named "@zyte/ds-<slug>" (kebab-case slug).`,
    );
  }
  return match[1]!;
}
...
export function buildTokensCss(
  ...
  const lines: string[] = [
    `/* Generated by @zyte/tokens-build. Do not edit by hand. */`,
...
export function composeDesignMd(
  ...
  const banner = [
    "<!--",
    "  This file is generated by @zyte/tokens-build.",
...
export async function buildPackage(
  ...
  // pulling in tsx as a runtime would force every consumer of @zyte/ds-* to
