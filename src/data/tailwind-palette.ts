export type TailwindShade = { shade: string; hex: string };
...
function family(name: string, hexes: readonly string[]): TailwindColorFamily {
  return {
    name,
    shades: SHADES.map((shade, i) => ({ shade, hex: hexes[i] })),
  };
}
